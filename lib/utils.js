...
async function fetchNewToken(oauth2, logger) {
  ...
  logger.trace('New token fetched : %o', authTokenResponse);
...
async function getValidToken(cfg, logger, emitter) {
  ...
  const newKeys = await fetchNewToken(oauth2, logger);
  newKeys.tokenExpiryTime = (new Date(tokenRefreshStartTime.getTime()
      + (newKeys.expires_in * 1000))).toISOString();
  if (emitter && emitter.emit) {
    emitter.emit('updateKeys', newKeys);
  }
  return newKeys.access_token;
}
